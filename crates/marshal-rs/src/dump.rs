...
impl<'a> Dumper<'a> {
    ...
    /// Allocate a phantom object slot to match the loader's object counting.
    /// The loader's read_data creates an entry for both the wrapper AND the
    /// inner value read via read_next. The dump only has one Value, so we
    /// must reserve a slot for the inner value the loader will create.
    fn reserve_phantom_object(&mut self) {
        // Use a key that can never collide with a real Value id (usize::MAX)
        // We just need to advance the position counter.
        let pos = self.objects.len();
        // Insert with a unique phantom key that won't collide
        self.objects.insert(usize::MAX - pos, pos);
    }
    ...
    fn write_structure(&mut self, mut value: Value) {
        ...
        if value.is_data() {
            ...
            // Bug fix: Check for object deduplication (ObjectLink) before
            // writing, and register this value in the object table to match
            // the loader's read_data which always pushes to objects.
        ...
        // Bug fix: Symbol values are NOT in the object table during load
        // (they use a separate symbols table). Handle them before the
        // general object-tracked branch to avoid inflating object indices.
        if let ValueType::Symbol(ref str) = *value {
            self.write_symbol(str);
            return;
        }
        ...
        match *value {
            ...
            _ => {
                ...
                // Bug fix: For Data, UserClass, and UserMarshal types, the
                // loader's read_data creates TWO object entries: one for the
                // wrapper (already registered above) and one for the inner
                // value read via read_next(). We must reserve a phantom slot
                // to keep indices aligned. UserDefined is excluded because
                // its read_data path uses read_chunk (not read_next), so no
                // extra entry is created.
                if value.is_data() || value.is_user_class() || value.is_user_marshal() {
                    self.reserve_phantom_object();
                }
                ...
                match *value {
                    ...
                    | ValueType::Bool(_)
                    | ValueType::Integer(_)
                    | ValueType::Symbol(_) => unreachable!(),
